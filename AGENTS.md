...
This repository contains a Vietnamese stock valuation platform. The Next.js 16 frontend lives in `frontend-next/`, with routes in `src/app`, UI in `src/components`, shared API/types/utilities in `src/lib`, and static assets in `public`. Python entry points and data scripts live at the repo root, including `run_pipeline.py` and `automation/update_excel_data.py`. The Flask backend is under `backend/` when available. Documentation is in `docs/`; generated SQLite files such as `vci_price_history.sqlite` are runtime artifacts unless a change explicitly requires updating them.
...
- `python run_pipeline.py`: refresh the security master and derived valuation cache.
