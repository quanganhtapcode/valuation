...
(crontab -l 2>/dev/null \
  ...
  | tr -d '\r'; \
  printf '%s\n' "$CRON_SCREENER" "$CRON_STATS_FINANCIAL" "$CRON_SHAREHOLDERS" "$CRON_NEWS" "$CRON_RATIO_DAILY" "$CRON_PRICE_HISTORY" "$CRON_TELEGRAM" "$CRON_VALUATION" "$CRON_FOREIGN" "$CRON_COMPANY" \
) | crontab -
