...
(crontab -l 2>/dev/null \
  | grep -v -E "fetch_vci_screener\.py|fetch_vci_stats_financial\.py|fetch_vci_shareholders\.py|fetch_vci_market_news\.py|fetch_vci_news\.py|fetch_vci_standouts\.py|fetch_vci_ratio_daily\.py|update_price_history(\.py)?|backend\.updater\.update_price_history|telegram_uptime_report\.sh|fetch_vci_valuation\.py|fetch_vci_foreign\.py|fetch_vci_company\.py|automation/vci_safe_run\.sh" \
  | tr -d '\r'; \
  printf '%s\n' "$CRON_SCREENER" "$CRON_STATS_FINANCIAL" "$CRON_FINANCIALS" "$CRON_SHAREHOLDERS" "$CRON_NEWS" "$CRON_RATIO_DAILY" "$CRON_PRICE_HISTORY" "$CRON_TELEGRAM" "$CRON_VALUATION" "$CRON_FOREIGN" "$CRON_COMPANY" \
) | crontab -
