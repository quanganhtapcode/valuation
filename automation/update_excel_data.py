...
# Compatibility:
# - EXCEL_MAX_WORKERS controls both phases (legacy)
# - EXCEL_DOWNLOAD_WORKERS / EXCEL_UPLOAD_WORKERS can override each phase
MAX_WORKERS = _env_int("EXCEL_MAX_WORKERS", DEFAULT_WORKERS)
DOWNLOAD_WORKERS = _env_int("EXCEL_DOWNLOAD_WORKERS", MAX_WORKERS)
...
def run_update(
    ...
    def notify_progress(message: str) -> None:
        ...
        try:
            ...
            print(f"[TG] progress notify failed: {exc}")

    print("=" * 70)
    print("  📊 WEEKLY VIETCAP EXCEL → CLOUDFLARE R2")
    print(
        f"  ⚡ DL workers={DOWNLOAD_WORKERS} | UP workers={UPLOAD_WORKERS} "
        f"| retries={MAX_RETRIES} | interval {UPDATE_INTERVAL_DAYS}d"
    )
    print("=" * 70)
    ...
    print(f"\nSTARTING DOWNLOAD PHASE ({DOWNLOAD_WORKERS} workers)...\n")
    ...
    with ThreadPoolExecutor(max_workers=DOWNLOAD_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            if content is not None:
                ...
                reached = len(downloaded_files) // 100
                while reached > dl_milestone:
                    dl_milestone += 1
                    notify_progress(
                        f"📥 Tiến độ tải dữ liệu: đã xong {dl_milestone * 100}/{total} mã."
                    )
            if token_expired:
    ...
    if downloaded_files:
        print(f"\nSTARTING UPLOAD PHASE ({UPLOAD_WORKERS} workers)...\n")
        up_milestone = 0
        with ThreadPoolExecutor(max_workers=UPLOAD_WORKERS) as executor:
            ...
            for future in as_completed(upload_futures):
                future.result()
                with counter_lock:
                    uploaded_ok = r2_success_count
                reached = uploaded_ok // 100
                while reached > up_milestone:
                    up_milestone += 1
                    notify_progress(
                ...
                def run_job(tkn: str, st: dict) -> None:
                    ...
                    try:
                        ...
                        try:
                            ...
                            send_telegram_message(
                                bot_token, chat_id,
                                "✅ Thu thập forecast PE/PB hoàn tất.",
                            )
                        except Exception as exc:
                            print(f"[JOB] Forecast fetch error: {exc}", flush=True)
                            send_telegram_message(
                                bot_token, chat_id,
                                f"⚠️ Forecast fetch lỗi: {exc}",
                            )
                    except Exception as exc:
