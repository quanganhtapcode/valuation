...
def _sanitize(text: str) -> str:
    """Normalize Unicode chars that cause Gemma API 500 errors."""
    if not text:
        return text
    replacements = {
        "“": '"', "”": '"',  # curly double quotes
    ...
    # Drop any remaining non-ASCII, non-Latin, non-Vietnamese characters
    # Keep basic Latin + Vietnamese Unicode block (U+0000-U+024F) + common CJK
    text = re.sub(r"[^\x00-ɏḀ-ỿ -⁯]", "", text)
    return text
        ...
        for attempt in range(3):  # up to 3 attempts per model
            try:
                ...
                if e.code in _TRANSIENT_ERRORS and attempt < 2:
                    ...
                    logger.warning(f"Model {model} HTTP {e.code} (attempt {attempt+1}), retrying in {wait}s")
                    time.sleep(wait)
                    last_err = e
                    continue
                last_err = e
