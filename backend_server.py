...
class StockDataProvider:
    def __init__(self):
        ...
        self._data_file = 'all_stock_data.json'
    ...
    def _load_industry_data(self, industry: str) -> dict:
        ...
        try:
            ...
            if found_filename:
                ...
                logger.debug(f"Searched for: stock_data_{normalized_industry}.json and stock_data_{industry}.json")
                if os.path.exists(self._industry_data_folder):
                    available_files = [f for f in os.listdir(self._industry_data_folder) if f.endswith('.json')]
                    logger.debug(f"Available files: {available_files[:5]}...")  # Show first 5 files
                return {}
    ...
    def _process_quarter_data(self, quarter_data: dict, symbol: str, company_info: dict) -> dict:
            ...
            if 'ratios' in quarter_data:
                ...
                if ('Chỉ tiêu định giá', 'Outstanding Share (Mil. Shares)') in ratios.index:
                    ...
                    if pd.notna(shares_value):
                        ...
                        logger.debug(f"Shares outstanding from ratios: {processed['shares_outstanding']}")
                    ...
                    if pd.notna(shares_value):
                        processed['shares_outstanding'] = float(shares_value) * 1000000  # Convert from millions
                        logger.debug(f"Shares outstanding from ratios (alt): {processed['shares_outstanding']}")
                ...
                if 'issue_share' in overview.index and not processed.get('shares_outstanding'):
                    ...
                    if pd.notna(shares_value):
                        ...
                        if shares_float > 1e12:  # If larger than 1 trillion, likely in wrong unit
                            ...
                            logger.debug(f"Adjusted shares outstanding from overview: {shares_float}")
                        processed['shares_outstanding'] = shares_float
                        logger.debug(f"Shares outstanding from overview: {processed['shares_outstanding']}")
        ...
        if 'shares_outstanding' in processed:
            ...
            if shares > 1e12:
                ...
                logger.debug(f"Adjusted shares outstanding from {shares} to {processed['shares_outstanding']}")
            # Log final shares outstanding for debugging
            logger.debug(f"Final shares outstanding for {symbol}: {processed['shares_outstanding']}")
        
        # Log key metrics for debugging
        key_metrics = ['revenue', 'net_income', 'total_assets', 'total_equity', 'pe_ratio', 'pb_ratio', 'roe', 'roa']
        debug_info = {k: processed.get(k) for k in key_metrics if k in processed}
        logger.debug(f"Key quarter metrics for {symbol}: {debug_info}")
            ...
            if all(pd.notna(processed[key]) for key in ['operating_cash_flow', 'shares_outstanding', 'current_price']):
                if processed['shares_outstanding'] > 0 and processed['operating_cash_flow'] != 0:
                    ...
                    if cash_flow_per_share > 0:
                        ...
                        logger.debug(f"Calculated P/CF ratio: {processed['pcf_ratio']}")
            ...
            if pd.notna(processed['operating_cash_flow']) and pd.notna(processed['shares_outstanding']) and processed['shares_outstanding'] > 0:
                ...
                if current_price and pd.notna(current_price):
                    ...
                    if cash_flow_per_share > 0:
                        ...
                        logger.debug(f"Calculated P/CF ratio (alt): {processed['pcf_ratio']}")
...
def get_historical_chart_data(symbol):
    try:
        ...
        print(f"[DEBUG] income_quarter.columns: {income_quarter.columns}")
        print(f"[DEBUG] balance_quarter.columns: {balance_quarter.columns}")
        ...
        for index, row in last_20_quarters.iterrows():
            ...
            if industry == "Ngân hàng":
                ...
                try:
                    ...
                    print(f"[NIM DEBUG] {symbol} {period_label} Exception: {e}")
                    nim_val = np.nan
                print(f"[NIM DEBUG] {symbol} {period_label} FINAL nim_val: {nim_val}")
...
if __name__ == "__main__":
    ...
    print(f"Vietnamese Stock Valuation Backend – running on http://0.0.0.0:{port}")
