...
class StockDataProvider:
    def __init__(self):
        ...
        self._industry_name_mapping = {}
        self._load_industry_name_mapping()
        self._load_stock_data()
        self._load_csv_data()
        logger.info("StockDataProvider initialized with file-based data loading")

    def _load_industry_name_mapping(self):
        """Load industry name mapping from JSON file"""
        try:
            mapping_file = 'industry_name_mapping.json'
            if os.path.exists(mapping_file):
                with open(mapping_file, 'r', encoding='utf-8') as f:
                    mapping_data = json.load(f)
                    self._industry_name_mapping = mapping_data.get('vietnamese_to_ascii', {})
                    logger.info("Loaded industry name mapping")
            else:
                logger.warning(f"Industry name mapping file {mapping_file} not found")
                self._industry_name_mapping = {}
        except Exception as e:
            logger.error(f"Failed to load industry name mapping: {e}")
            self._industry_name_mapping = {}

    def _load_stock_data(self):
        """Load stock data from industry files"""
        try:
            # Since we only have industry-specific files, we'll set up basic structures
            self._stock_data_cache = {}
            self._all_symbols = []
            self._industry_mapping = {}
            
            # Load all industry files to build symbol lists and mapping
            if os.path.exists(self._industry_data_folder):
                industry_files = [f for f in os.listdir(self._industry_data_folder) if f.startswith('stock_data_') and f.endswith('.json')]
                total_symbols = 0
                
                for filename in industry_files:
                    # Extract industry name from filename
                    industry_name = filename[11:-5]  # Remove "stock_data_" and ".json"
                    filepath = os.path.join(self._industry_data_folder, filename)
                        ...
                        with open(filepath, 'r', encoding='utf-8') as f:
                            ...
                            if 'stocks' in industry_data:
                                ...
                                for symbol, data in stocks.items():
                                    ...
                                    self._industry_mapping[symbol] = industry_name
                                    total_symbols += 1
                    except Exception as e:
                        logger.warning(f"Error loading {filename}: {e}")
                
                logger.info(f"Loaded stock data for {total_symbols} symbols from {len(industry_files)} industry files")
                return True 
            else:
                logger.warning(f"Industry data folder {self._industry_data_folder} not found!")
                return False
    ...
    def _get_industry_json_filename(self, industry: str) -> str:
        """Get the JSON filename for a given industry using ASCII mapping"""
        # First try to map Vietnamese name to ASCII
        ascii_name = self._industry_name_mapping.get(industry)
        if ascii_name:
            ascii_filename = f"stock_data_{ascii_name}.json"
            ascii_filepath = os.path.join(self._industry_data_folder, ascii_filename)
            if os.path.exists(ascii_filepath):
                return ascii_filename
        
        # Fallback: try original filename
        original_filename = f"stock_data_{industry}.json"
        original_filepath = os.path.join(self._industry_data_folder, original_filename)
        if os.path.exists(original_filepath):
            return original_filename
        
        # If neither found, try searching directory
        try:
            if os.path.exists(self._industry_data_folder):
                for filename in os.listdir(self._industry_data_folder):
                    if filename.startswith("stock_data_") and filename.endswith(".json"):
                        ...
                        if (industry_from_file == industry or 
                            ...
                            industry_from_file == ascii_name):
                            return filename
        except Exception as e:
            logger.warning(f"Error searching for industry file: {e}")
        
        # Return the ASCII filename even if not found (for logging purposes)
        return ascii_filename if ascii_name else original_filename

    def _normalize_industry_name(self, industry: str) -> str:
        """Convert Vietnamese industry name to ASCII filename format using mapping"""
        # Try direct mapping first
        ascii_name = self._industry_name_mapping.get(industry)
        if ascii_name:
            return ascii_name
            
        # Fallback: manual normalization (for any unmapped names)
        vietnamese_map = {
            'á': 'a', 'à': 'a', 'ả': 'a', 'ã': 'a', 'ạ': 'a',
    ...
    def _load_industry_data(self, industry: str) -> dict:
        ...
        try:
            # Use the improved filename resolution method
            found_filename = self._get_industry_json_filename(industry)
            ...
            if os.path.exists(filepath):
                ...
                logger.warning(f"Industry data file not found for: {industry} (expected: {found_filename})")
                return {}
...
if __name__ == "__main__":
    ...
    logger.info(f"Vietnamese Stock Valuation Backend – running on http://0.0.0.0:{port}")
