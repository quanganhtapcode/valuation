...
def get_historical_chart_data(symbol):
    try:
        ...
        for index, row in last_20_quarters.iterrows():
            ...
            pe_val = float(pe_ratio) if pd.notna(pe_ratio) else 0
            pb_val = float(pb_ratio) if pd.notna(pb_ratio) else 0
            ...
            historical_data["pe_data"].append(pe_val)
            historical_data["pb_data"].append(pb_val)
