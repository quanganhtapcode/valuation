...
def ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    """)
    # Add new columns to existing tables (safe — ignored if already present)
    _new_cols = [
        ("price_to_cash_flow", "REAL"), ("ev_to_ebitda", "REAL"), ("roic", "REAL"),
        ("ebit_margin", "REAL"), ("pre_tax_margin", "REAL"),
        ("current_ratio", "REAL"), ("quick_ratio", "REAL"), ("cash_ratio", "REAL"),
        ("debt_to_equity", "REAL"), ("financial_leverage", "REAL"), ("asset_turnover", "REAL"),
        ("dividend_yield", "REAL"), ("market_cap", "REAL"), ("shares", "REAL"),
        ("day_sale_outstanding", "REAL"), ("days_inventory_outstanding", "REAL"),
        ("days_payable_outstanding", "REAL"),
    ]
    ...
    for row in entries:
        ...
        conn.execute("""
            ...
            ON CONFLICT(ticker, year_report, quarter_report) DO UPDATE SET
              ...
              day_sale_outstanding       = excluded.day_sale_outstanding,
        ...
        """, (
            ...
            _to_float(row.get("roic")),
            _to_float(row.get("grossMargin")),
            _to_float(row.get("ebitMargin")),
            ...
            _to_float(row.get("dividendYield")),
            _to_float(row.get("marketCap")),
            _to_float(row.get("numberOfSharesMktCap")),
            _to_float(row.get("daySaleOutstanding")),
            _to_float(row.get("daysInventoryOutstanding")),
            _to_float(row.get("daysPayableOutstanding")),
