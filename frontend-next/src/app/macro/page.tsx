...
function RateCard({ item, isVnd }: { item: RateItem; isVnd: boolean }) {
    ...
    return (
        <Card className="p-4">
            ...
            {item.unit && (
                <p className="text-[11px] text-tremor-content dark:text-dark-tremor-content mt-0.5">
                    {item.unit}
                </p>
            )}
...
function ComingSoonCard({ title, desc }: { title: string; desc: string }) {
    return (
        <Card className="p-5 flex flex-col gap-1 border-dashed opacity-60">
            <p className="font-semibold text-sm text-tremor-content-strong dark:text-dark-tremor-content-strong">
                {title}
            </p>
            ...
            <div className="max-w-[1600px] mx-auto p-4 md:p-6 space-y-10">
                ...
                <div>
                    <h1 className="text-3xl md:text-4xl font-bold leading-tight tracking-tight">
                        Kinh Tế{' '}
                        <span className="text-blue-600 dark:text-blue-400">Vĩ Mô</span>
                    </h1>
                ...
                <section>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        {loading
                            ...
                            : fxRates.map((item) => (
                                <RateCard key={item.symbol} item={item} isVnd={true} />
                            ))}
                    </div>
                </section>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        {loading
                            ...
                            : commodities.map((item) => (
                                <RateCard key={item.symbol} item={item} isVnd={false} />
                            ))}
                    </div>
