...
export default function StockDetailClient({ initialStockInfo }: { initialStockInfo: StockInfo }) {
    const symbol = initialStockInfo.symbol;
    const { lang } = useLanguage()
    const t = translations[lang]

    const [stockInfo, setStockInfo] = useState<StockInfo | null>(initialStockInfo);
    ...
    useEffect(() => {
        ...
        async function loadData() {
            ...
            try {
                ...
                // Retry a missing profile independently; never queue it behind summary.
                if (!initialStockInfo.overview?.description) fetch(`/api/company-profile/${symbol}?lang=${lang}`, { signal: controller.signal })
                    .then(r => r.ok ? r.json() : null)
                    .then(res => {
                ...
                // Identity and company profile are already present in server-rendered HTML.
                const stockRes = await fetch(`${API.STOCK_SUMMARY(symbol)}?realtime=0`, { signal: controller.signal })
                    .then(r => r.ok ? r.json() : null)
                    .catch(() => null);
    ...
    return (
        <div className={styles.container}>
            {/* ── Stock quote header ───────────────────────────────── */}
            <section aria-busy={isLoading} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-[#111827]">
