    ...
    return (
        <div className="overflow-x-auto">
            <table className="w-full text-[11px]">
                ...
                <tbody>
                    {rows.map((r, i) => {
                        const rg = r.revenue_growth;
                        const pg = r.profit_growth;
                        return (
                            <tr key={i} className={r.is_forecast ? 'text-gray-400 dark:text-gray-500 italic' : 'text-gray-700 dark:text-gray-300'}>
                                ...
                                <td className={`py-0.5 pr-3 text-right ${rg != null && rg >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-500 dark:text-rose-400'}`}>
                                    {fmtPct(rg)}
                                </td>
                                <td className={`py-0.5 text-right ${pg != null && pg >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-500 dark:text-rose-400'}`}>
                                    {fmtPct(pg)}
                                </td>
                            </tr>
                        );
...
function ValuationGrid({ v }: { v: Valuation }) {
    const rows = [
        { label: 'P/E TTM', val: fmtX(v.pe_ttm), avg2: fmtX(v.pe_2yr_avg), avg5: fmtX(v.pe_5yr_avg), sector: fmtX(v.pe_sector), note: v.pe_commentary },
        { label: 'P/B TTM', val: fmtX(v.pb_ttm), avg2: fmtX(v.pb_2yr_avg), avg5: fmtX(v.pb_5yr_avg), sector: fmtX(v.pb_sector), note: v.pb_commentary },
    ];
    return (
...
function LongTermGrid({ lt }: { lt: LongTerm }) {
    const items = [
        { label: 'Tăng trưởng EPS (CAGR 3 năm)', value: fmtPct(lt.eps_cagr_3yr), badge: lt.eps_cagr_label },
        { label: 'Lợi suất cổ tức TB', value: fmtPct(lt.dividend_yield_avg), badge: lt.dividend_yield_label },
        { label: 'ROE trung bình', value: fmtPct(lt.roe_avg), badge: lt.roe_label },
    ];
    return (
        <div className="overflow-x-auto">
            <table className="w-full text-[11px]">
                <tbody>
                    {items.map(r => (
                        <tr key={r.label} className="border-b border-blue-100/40 dark:border-blue-900/20 last:border-0">
                            <td className="py-1 pr-3 text-gray-600 dark:text-gray-400">{r.label}</td>
                            <td className="py-1 pr-2 text-right font-semibold text-gray-800 dark:text-gray-200">{r.value}</td>
                            <td className="py-1 text-right">
                                {r.badge && (
                                    <span className="text-[9px] bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 rounded px-1.5 py-0.5">
                                        {r.badge}
                                    </span>
                                )}
                            </td>
...
export default function AiAnalysisCard({ symbol }: { symbol: string }) {
    ...
    let parsed: RichAnalysis | null = null;
    if (data.analysis_json) {
        try {
            parsed = JSON.parse(data.analysis_json) as RichAnalysis;
        } catch {
            parsed = null;
        }
    }

    const isRich = parsed && (parsed.technical || parsed.recommendation || parsed.growth_table || parsed.valuation);
        ...
        <div className="rounded-xl bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 border border-blue-100 dark:border-blue-900/40 p-4 space-y-4">
            ...
            {isRich ? (
                <>
                    ...
                    {parsed?.technical && (
                        <div>
                            <SectionTitle>Trạng thái kỹ thuật</SectionTitle>
                            <p className="text-[12px] leading-relaxed text-gray-600 dark:text-gray-400">
                                {parsed.technical.trend}
                            </p>
                            {(parsed.technical.support || parsed.technical.resistance) && (
                                <div className="mt-1.5 flex gap-3 text-[11px]">
                                    {parsed.technical.support && (
                                        <span className="text-gray-500 dark:text-gray-400">
                                            Hỗ trợ: <strong className="text-emerald-600 dark:text-emerald-400">{parsed.technical.support.toLocaleString('vi-VN')}</strong>
                                        </span>
                                    )}
                                    {parsed.technical.resistance && (
                                        <span className="text-gray-500 dark:text-gray-400">
                                            Kháng cự: <strong className="text-rose-500 dark:text-rose-400">{parsed.technical.resistance.toLocaleString('vi-VN')}</strong>
                                        </span>
                                    )}
                                </div>
                            )}
                    ...
                    {parsed?.recommendation && (
                        <div>
                            <SectionTitle>Khuyến nghị & Giá mục tiêu</SectionTitle>
                    ...
                    {parsed?.long_term && (
                        <div>
                            <SectionTitle>Dài hạn</SectionTitle>
                            <LongTermGrid lt={parsed.long_term} />
                        </div>
                    )}
                </>
            ) : parsed?.key_issues ? (
                /* Fallback: legacy Bull/Bear format */
