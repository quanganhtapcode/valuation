        ...
        <div className={`rounded-xl border border-slate-200 bg-white p-4 shadow-sm ring-1 ${toneRingClass(tone)} dark:border-slate-800 dark:bg-slate-900`}>
            <div className="flex items-start justify-between gap-3">
                <div>
                    ...
                    <p className="mt-1 text-base font-semibold text-slate-900 dark:text-slate-100">{ratingLabel(data?.rating)}</p>
                </div>
            ...
            <div className="mt-3 space-y-2.5">
                <div className="flex items-end justify-between">
                    <span className="text-xs text-slate-500 dark:text-slate-400">Sức mạnh tín hiệu</span>
                ...
                <div className="flex flex-wrap items-center gap-2 text-[11px]">
                    <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">Buy: {buy}</span>
                    <span className="rounded-full bg-slate-100 px-2 py-0.5 text-slate-600 dark:bg-slate-800 dark:text-slate-300">Neutral: {neutral}</span>
                    <span className="rounded-full bg-rose-50 px-2 py-0.5 text-rose-700 dark:bg-rose-500/10 dark:text-rose-300">Sell: {sell}</span>
                    {totalSignals > 0 && (
                        <span className="ml-auto text-slate-500 dark:text-slate-400">{totalSignals} tín hiệu</span>
                    )}
                </div>
                {entries.length > 0 && (
    ...
    return (
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="mb-3">
                ...
                <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">Mức kháng cự và hỗ trợ quan trọng theo Pivot Point.</p>
            </div>
        ...
        <div className="space-y-5">
            ...
            <div className="rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Phân tích kỹ thuật</h3>
                        <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                            Tín hiệu tổng hợp theo dao động, trung bình động và vùng hỗ trợ/kháng cự.
                        </p>
                    </div>
            ...
            {!loading && data && (
                <div className="space-y-4">
                    <div className="grid gap-4 md:grid-cols-3">
                        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                            <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Đánh giá tổng hợp</p>
                            <p className={`mt-1 text-lg font-semibold ${summaryToneClass}`}>{ratingLabel(data.gaugeSummary?.rating)}</p>
                        </div>
                        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                            <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Giá hiện tại</p>
                            <p className="mt-1 text-lg font-semibold tabular-nums text-slate-900 dark:text-slate-100">{valueText(data.price)}</p>
                        </div>
                        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                            <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Cập nhật</p>
                            <p className="mt-1 text-sm font-medium text-slate-900 dark:text-slate-100">
                                {formatDateTime(data.matchTime || current?.fetched_at_utc || current?.serverDateTime)}
                            </p>
                        </div>
                    </div>
